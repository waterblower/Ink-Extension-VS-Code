...
const findRootInkFile = (docPath) => {
    ...
    for (const root of roots) {
        if (collectInkFiles(root).has(docPath)) {
            return root;
        }
    }
        ...
        for (let i = 0; i < lines.length; i++) {
            for (const target of parseDivertTargets(lines[i])) {
                ...
                if (exactMatch || dottedMatch) {
                    ...
                    const range = new Range(i, col, i, col + name.length);
                    locations.push(new Location(Uri.file(filePath), range));
                }
            }
        }
    }
    ...
    for (const filePath of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (!assignmentLoc && assignRegex.test(searchIn)) {
                const col = searchIn.indexOf(name);
                const range = new Range(i, col, i, col + name.length);
                assignmentLoc = new Location(Uri.file(filePath), range);
            }
        }
    }
    ...
    for (const filePath of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            while ((m = re.exec(searchIn)) !== null) {
                const col = m.index;
                let isLogic = false;
                if (/^\s*(?:VAR|LIST)\s/.test(searchIn)) {
                    isLogic = true;
                } else if (searchIn.lastIndexOf("~", col) !== -1) {
                    isLogic = true;
                } else {
...
const getContextInfoAtPosition = (document, position) => {
    ...
    if (commentIdx !== -1 && col >= commentIdx) {
        return null;
    }
...
const foldingProvider = {
    provideFoldingRanges(document) {
        ...
        for (let i = 0; i < headers.length; i++) {
            ...
            if (end > start) {
                ranges.push(
                    new FoldingRange(start, end, FoldingRangeKind.Region),
                );
            }
        }
        return ranges;
...
const imageLinkProvider = {
    provideDocumentLinks(document, _) {
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            const link = new DocumentLink(range, Uri.file(absPath));
